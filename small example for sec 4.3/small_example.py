...
def get_table6():
    ...
    for m in designs:
        with open("Table6_part2.txt", "a") as f:
            print(m, file=f)
        qk_pairs = [(q,k) for q in [1,2,4,6,9] for k in [1,2,3,4,5,6]]
        result = {(q,k): reject_prob_parrell(covariates, k, q, 1280, tau=0.1, ntrials=100, more=False, design=m) for q, k in qk_pairs}
        results_null.append(result)
        for q in [1,2,4,6,9]:
            for k in [1,2,3,4,5,6]:
                with open("Table6_part2.txt", "a") as f:
...
def get_saved_results():
    ...
    for i, (q,k) in enumerate(model_specs):
        result = {}
        def processInput(t):
            c = reject_prob_parrell(covariates, k, q, 1280, tau=t, ntrials=100, more=False, design='C')
            s4 = reject_prob_parrell(covariates, k, q, 1280, tau=t, ntrials=100, more=False, design='S4')
            #with open("sim3_runtime.txt", "a") as f:
            #    print("start mt", file=f)
            mt = reject_prob_parrell(covariates, k, q, 1280, tau=t, ntrials=100, more=False, design='MT')
            mt2 = reject_prob_parrell(covariates, k, q, 1280, tau=t, ntrials=100, more=True, design='MT')
            #with open("sim3_runtime.txt", "a") as f:
